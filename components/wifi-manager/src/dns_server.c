...
void dns_server_task(void *pvParameters)
{
    ...
    while (1) {
        ...
        socket_fd = socket(addr_family, SOCK_DGRAM, ip_protocol);
        if (socket_fd < 0) {
        ...
        int err = bind(socket_fd, (struct sockaddr *)&dest_addr, sizeof(dest_addr));
        if (err < 0) {
        ...
        while (1) {
            ...
            int len = recvfrom(socket_fd, rx_buffer, sizeof(rx_buffer) - 1, 0, (struct sockaddr *)&source_addr, &socklen);
            ...
            // Data received
                ...
                if (reply_len <= 0) {
                    ...
                    int err = sendto(socket_fd, reply, reply_len, 0, (struct sockaddr *)&source_addr, sizeof(source_addr));
                    if (err < 0) {
...
void start_dns_server(void) {
	if(task_dns_server == NULL){
        xTaskCreate(dns_server_task, "dns_server", 4096, NULL, CONFIG_WIFI_MANAGER_TASK_PRIORITY-1, &task_dns_server);
    }
}
...
void stop_dns_server(void) {
	if(task_dns_server){
        ...
        ESP_LOGI(TAG, "Shutting down dns server");
	}
}